...
class IterativeErasure(LDPCDecoder):

    """Iterative Erasure error correction scheme."""
    def __init__(self, ldpc_graph: nx.Graph, maxiter=30):
    ...
    def decode(self, y: np.ndarray) -> np.ndarray:
        ...
        for _ in range(self.maxiter):
            ...
            for edge in self.ldpc_graph.edges:
                ...
                self.ldpc_graph[V][C]['beta'] = np.prod(in_messages, axis=0)
            ...
            for edge in self.ldpc_graph.edges:
                ...
                self.ldpc_graph[V][C]['alpha'] += self.ldpc_graph.nodes[V]['gamma']
                self.ldpc_graph[V][C]['alpha'] = np.sign(self.ldpc_graph[V][C]['alpha']).astype(int)
